...
const PointsReviewPanel = ({ currentUser }) => {
  ...
  return (
    <div className="min-h-screen p-6 flex flex-col">
      <h3 className="text-xl font-bold text-white mb-4">📋 積分審核管理</h3>

      {loading ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-400 mx-auto"></div>
            <p className="mt-2 text-slate-300">載入中...</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          {(pendingEntries || []).map((entry) => (
            <div key={entry?.id || Math.random()} className="bg-slate-700/30 backdrop-blur-sm border border-slate-600/50 rounded-lg p-6 shadow-sm">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-lg font-semibold text-white">{entry?.standardName || '未知項目'}</h4>
                  <p className="text-sm text-slate-300">
                    {entry?.employeeName || '未知員工'} • {entry?.department || '未知部門'} • {entry?.submittedAt || '未知時間'}
                  </p>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-blue-400">{entry?.pointsCalculated || 0} 積分</div>
                  <div className="text-xs text-slate-400">計算後積分</div>
                </div>
              </div>

              <div className="mb-4">
                <h5 className="text-sm font-medium text-slate-200 mb-2">工作說明：</h5>
                <p className="text-sm text-slate-300 bg-slate-600/30 p-3 rounded">{entry?.description || '無說明'}</p>
              </div>

              {entry?.evidenceFiles && Array.isArray(entry.evidenceFiles) && entry.evidenceFiles.length > 0 && (
                <div className="mb-4">
                  <h5 className="text-sm font-medium text-slate-200 mb-2">證明文件：</h5>
                  <div className="flex flex-wrap gap-2">
                    {entry.evidenceFiles.map((file, index) => (
                      <span key={index} className="inline-flex items-center px-2 py-1 bg-blue-600/20 text-blue-300 text-xs rounded">
                        📎 {file}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex justify-end space-x-3">
                <button
                  ...
                  className="px-4 py-2 border border-red-400/50 text-red-400 rounded-md hover:bg-red-600/20 transition-colors"
                >
                  ❌ 拒絕
                </button>
                <button
